...
def get_mel_from_wav(audio, norm=True):
    clipt = False
    sampling_rate = hparams.sampling_rate
    if sampling_rate != _stft.sampling_rate:
        raise ValueError("{} SR doesn't match target {} SR".format(
            sampling_rate, _stft.sampling_rate))
    audio_norm = audio / hparams.max_wav_value if norm else audio
    audio_norm = audio_norm.unsqueeze(0)
    audio_norm = torch.autograd.Variable(audio_norm, requires_grad=False)
    if not norm:
        pre_min = torch.min(audio_norm.data)
        audio_norm = torch.clamp(audio_norm, -1, 1)
        if pre_min != torch.min(audio_norm.data):
            clipt = True
    melspec, energy = _stft.mel_spectrogram(audio_norm)
    melspec = torch.squeeze(melspec, 0)
    energy = torch.squeeze(energy, 0)

    return melspec, energy, clipt
